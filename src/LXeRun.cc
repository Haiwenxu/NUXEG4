  ...
  : G4Run()
{
  ...
  fSiPMsAboveThreshold = fSiPMsAboveThreshold2 = 0;
...
void LXeRun::Merge(const G4Run* run)
{
  ...
  fSiPMsAboveThreshold += localRun->fSiPMsAboveThreshold;
  fSiPMsAboveThreshold2 += localRun->fSiPMsAboveThreshold2;
...
void LXeRun::EndOfRun()
{
  ...
  //G4Run:: [numberOfEvent] : input variable of beamOn
  ...
  G4double hitsAbove     = G4double(fSiPMsAboveThreshold) / n_evt;
  G4double hitsAbove2    = G4double(fSiPMsAboveThreshold2) / n_evt;
