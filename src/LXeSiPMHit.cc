...
LXeSiPMHit::LXeSiPMHit(const LXeSiPMHit& right)
  : G4VHit()
{
  fSipmNumber = right.fSipmNumber;
...
G4bool LXeSiPMHit::operator==(const LXeSiPMHit& right) const
{
  return (fSipmNumber == right.fSipmNumber);
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

void LXeSiPMHit::Draw()
{

  // std::cout << "============= " << fDrawit << "; " << fPhysVol << " ============" << std::endl;
  if(fDrawit && fPhysVol)
  // if(fDrawit)
  // if(fPhysVol)
  {  // Redraw only the SiPMs that have hit counts > 0
    // Also need a physical volume to be able to draw anything
    G4VVisManager* pVVisManager = G4VVisManager::GetConcreteInstance();
    if(pVVisManager)
    {  // Make sure that the VisManager exists
      std::cout << "============= VisManager exists ============" << std::endl;
      G4VisAttributes attribs(G4Colour(1., 0., 0.));
      attribs.SetForceSolid(true);
      G4RotationMatrix rot;
      if(fPhysVol->GetRotation())  // If a rotation is defined use it
        rot = *(fPhysVol->GetRotation());
      G4Transform3D trans(rot.inverse(), fPhysVol->GetTranslation());  // Create transform
